...
public:
  MyRobot()
 {
   ...
   hardware_interface::JointStateHandle state_handle_a("wheelchair_base_link_left_wheel_joint", &pos[0], &vel[0], &eff[0]); //pos vel eff outputs of the state message...
   jnt_state_interface.registerHandle(state_handle_a);
   hardware_interface::JointStateHandle state_handle_b("wheelchair_base_link_right_wheel_joint", &pos[1], &vel[1], &eff[1]); //pos vel eff outputs of the state message...
   jnt_state_interface.registerHandle(state_handle_b);


   hardware_interface::JointHandle pos_handle_a(jnt_state_interface.getHandle("wheelchair_base_link_left_wheel_joint"), &cmd[0]); //cmd is the commanded value depending on the controller.
   hardware_interface::JointHandle pos_handle_b(jnt_state_interface.getHandle("wheelchair_base_link_right_wheel_joint"), &cmd[1]); //cmd is the commanded value depending on the controller.
